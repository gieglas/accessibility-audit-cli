...
An Excel workbook is available for quick charting and reporting.
...
##### Excel: Copy latest aggregated CSV
The package has a **Copy latest aggregated CSV** helper command.

It copies a selected aggregated CSV file into the Excel analysis folder using a consistent filename.

The command uses a configuration file to specify the source and destination folders.
...
{
  "copyLatestCsv": {
    "sourceDir": "./analysis-output",
    "destDir": "./excel_analysis/aggregated_analysis",
    "destName": "aggregated-analysis.csv"
  }
}
```
...
```bash
npx accessibility-audit analysis copy-latest-csv --config analysis-config.json
```

You can optionally override the source and destination folders and destination filename:

```bash
npx accessibility-audit analysis copy-latest-csv --config analysis-config.json [--sourceDir audits_analysis] [--destDir excel] [--destName cyprus-accessibility-analysis-for-excel.csv]
```

For example:

```bash
npx accessibility-audit analysis copy-latest-csv --config analysis-config.json --sourceDir reports/aggregated_analysis --destDir excel_analysis/aggregated_analysis --destName aggregated-analysis.csv
```

This command:

- Lists available aggregated CSV files
- Prompts you to select one interactively
- Copies it to: `excel_analysis/aggregated_analysis/aggregated-analysis.csv`

Notes:

- The original CSV is never modified
- This is intended to support Excel-based analysis workflows
...
- The workbook contains a Power Query named `cyprus-accessibility-analysis`; open Excel, go to **Data → Queries & Connections**, right-click that query, and set its source to your latest aggregated CSV (for example the sample file above or an export you produce)
